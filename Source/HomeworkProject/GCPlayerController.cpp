...
void AGCPlayerController::SetupInputComponent()
{
	...
	//в состоянии паузы большинство инпут экшенов будут игнорированы.
	...
	ToggleMenuBinding.bExecuteWhenPaused = true;//Должна ли привязка выполняться во время паузы
}
...
void AGCPlayerController::OnInteractableObjectFound(FName ActionName)
{
	...
	//из инпута получаем кнопки для экшона
	...
	PlayerHUDWidget->SetHighlightInteractableVisibility(HasAnyKeys);//выставляем видимость в случае если найдены клавиши
}
...
void AGCPlayerController::ToggleMainMenu()
{
	...
	if (MainMenuWidget->IsVisible())
	{
		...
		//Структура данных, используемая для настройки режима ввода, 
		// который позволяет реагировать на ввод пользователя только игроку / контроллеру игрока.
	...
	else
	{
		...
		//Структура данных, используемая для настройки режима ввода, который позволяет пользовательскому интерфейсу реагировать на ввод данных пользователем.
		//Если пользовательский интерфейс не обрабатывает ввод данных, то у контроллера игрока есть такая возможность.
