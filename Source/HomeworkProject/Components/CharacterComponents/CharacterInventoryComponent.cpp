...
{
	Super::BeginPlay();
	InventorySlots.AddDefaulted(Capacity);//резервируем элементы массивов айтемов.
}
...
{
	//FindByPredicate: Это метод или функция, которая принимает предикат (условие) для поиска внутри контейнера.
